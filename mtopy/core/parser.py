...
class ASTBuilder:
        ...
        if self._match(TokenType.SPACE):
            ...
            if is_function_call:
                ...
                while self._is_still_in_expr():
                    if self._match(TokenType.SPACE):
                        input_params.append(String(Token(TokenType.STRING, ''.join([t.val for t in token_list]), token_list[0].ln, token_list[0].col)))
                        token_list = []
                    else:
                ...
                return FunctionCall(func_name, IndexExpression(input_params))
    ...
    def _parse_try_statement(self) -> Node:
        ...
        if self._match(TokenType.IDENTIFIER):
            exception = Identifier(self._previous())
        else:
            exception = None
    ...
    def _token_list_to_string(self, token_list: list[Token]) -> str:
        ...
        for token in token_list:
            if False and token.typ == TokenType.CHAR_ARRAY:
