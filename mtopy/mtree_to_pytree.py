...
class MPTreeConverter:
    ...
    def convert_tree(self, node: Tree.Node, ctx_typ=ast.Load()) -> Optional[ast.AST]:
        try:
            # Terminal symbols
            ...
            if isinstance(node, Tree.Program):
                ...
                ret_var = [ast.arg(arg=self.convert_tree(param).id) for param in node.output_params]
                if len(ret_var) == 1:
                ...
                return ast.FunctionDef(
                    ...
                    args=ast.arguments(
                        posonlyargs=[],
                        args=[ast.arg(arg=self.convert_tree(param).id) for param in node.input_params],
                        kwonlyargs=[],
                        kw_defaults=[],
                        defaults=[]
                    ),
                ...
                return ast.Lambda(
                    args=ast.arguments(
                        posonlyargs=[],
                        args=[ast.arg(arg=self.convert_tree(param).id) for param in node.parameters],
                        kwonlyargs=[],
                        kw_defaults=[],
                        defaults=[]
                    ),
                    body=self.convert_tree(node.body[0])  # Assuming single expression for lambda body
                )
            
            elif isinstance(node, Tree.IfStatement):
                return ast.If(
                    test=self.convert_tree(node.condition),
                ...
                return ast.For(
                    target=self.convert_tree(node.identifier),
                    iter=self.convert_tree(node.expression),
                    body=self._body_list_to_ast(node.body),
                    orelse=[]
                )
                ...
                return ast.Try(
                    ...
                    handlers=[ast.ExceptHandler(
                        type=self.convert_tree(node.exception) if node.exception is not None else None,
                        name=None,
                ...
                lineno = 1
                return ast.Assign(
                    targets=[self.convert_tree(lval, ast.Store()) for lval in node.lvalue],
                    value=self.convert_tree(node.rvalue),
                    lineno=lineno
                )
            
            elif isinstance(node, Tree.FunctionCall):
                if node.arguments is not None:
                    arguments = [self.convert_tree(arg) for arg in node.arguments.indices if arg is not None]
                else:
                    arguments = None
                ...
                return self.config.converter.access_mat(
                    self.convert_tree(node.identifier),
                    [self.convert_tree(index) for index in node.arguments.indices],
                )
                ...
                return self.config.converter.access_struct(
                    self.convert_tree(node.identifier),
                    [self.convert_tree(arg) for arg in node.arguments]
                )
            
            elif isinstance(node, Tree.MatrixExpression):
                return self.config.converter.create_mat([[self.convert_tree(elem) for elem in row] for row in node.elements])
            
            elif isinstance(node, Tree.CellArrayExpression):
                return self.config.converter.create_cell([[self.convert_tree(elem) for elem in row] for row in node.elements])

            elif isinstance(node, Tree.ColonArray):
                return self.config.converter.arange(
                    start=self.convert_tree(node.start),
                    stop=self.convert_tree(node.stop),
                    step=self.convert_tree(node.step) if node.step else None,
                )
                ...
                if op is not None:
                    ...
                    return convert_func(self.convert_tree(node.left), self.convert_tree(node.right))
                ...
                if op is not None:
                    return ast.UnaryOp(
                        op=op,
                        operand=self.convert_tree(node.operand)
                    )
                else:
                    convert_func = getattr(self.config.converter, node.operator)
                    return convert_func(self.convert_tree(node.operand))
    ...
    def _body_list_to_ast(self, body: list[Tree.Node]) -> list[ast.AST]:
        body = [self.convert_tree(stmt) for stmt in body if stmt is not None]
        ast_body = []
        for b in body:
            if b is None:
                continue
    ...
    def _convert_elseif_chain(self, elseif_clauses: list[Tree.ElseIfClause], else_body: Optional[list[Tree.Node]]) -> list[ast.AST]:
        ...
        return [ast.If(
            test=self.convert_tree(next_clause.condition),
            body=if_body,
    ...
    def _convert_switch_to_if(self, node: Tree.SwitchStatement) -> ast.If:
        ...
        for case in node.cases:
            test_list.append(
                ast.BinOp(
                    left=self.convert_tree(node.expression),
                    op=ast.Eq(),
                    right=self.convert_tree(case.condition)
                )
            )
