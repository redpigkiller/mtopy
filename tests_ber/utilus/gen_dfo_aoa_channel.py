...
def gen_dfo_aoa_channel(Q, config, preset_var):
    ...
    for i_Q in np.arange(1, n_Q + 1):
        selected_idxs = nchoosek[np.arange(1, Q_max + 1) - 1, Q[i_Q - 1] - 1]
        DFOq = np.zeros((np.size(selected_idxs, 1), Q[i_Q - 1]))
        AOAq = np.zeros((np.size(selected_idxs, 1), Q[i_Q - 1]))
        CHLq = np.zeros((np.size(selected_idxs, 1), Q[i_Q - 1], config['Lch'] + 1))
        
        for idx in np.arange(1, np.size(selected_idxs, 1) + 1):
            selected_idx = selected_idxs[idx - 1, : - 1]
